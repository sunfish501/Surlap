...
class _DayViewState extends ConsumerState<DayView> {
  ...
  void initState() {
    ...
    final startHour = du.isSameDay(date, now) ? (now.hour - 1).clamp(0, 20) : 7;
  ...
  Widget build(BuildContext context) {
    ...
    final allDay = [
      ...
      if (!filter.contains(birthdayThemeId))
        ...ref
            ...
            .map(
              (b) => EventItem(t: b.name, th: birthdayThemeId, birthday: true),
            ),
      // 공휴일(빨강 'holidays' 테마).
      ...
      if (!filter.contains(academicThemeId))
        ...dedupAcademicWithHolidays(
          ...
          (ref.watch(academicScheduleProvider)[widget.dateKey] ?? const [])
              .where(
                (n) => academicVisibleForGrade(n, NeisSchool.load()?.grade),
              ),
    ...
    final recurringForDay = showTt
        ? timetableSubjectsForDate(ref, date)
        : const <int, String>{};
    final lunchMenu = _isInCurrentWeek(date)
        ? neis.lunch[date.weekday - 1]?.trim()
        : null;
    // 시간/종일/할일/반복 전부 없으면 빈 날 → 마스코트 힌트.
    final dayEmpty =
        timed.isEmpty &&
        recurringForDay.isEmpty &&
        (lunchMenu == null || lunchMenu.isEmpty) &&
        allDay.isEmpty &&
        dayTodos.isEmpty;
        ...
        child: Column(
          ...
          children: [
            ...
            if (allDay.isNotEmpty)
              _AllDayBar(items: allDay, themes: themes, sh: sh),
            // 할 일
                                ...
                                children: [
                                  ...
                                  if (isToday)
                                    Positioned.fill(
                                      child: IgnorePointer(
                                        child: Container(
                                          color: sh.accent.withValues(
                                            alpha: sh.dark ? 0.06 : 0.04,
                                          ),
                                        ),
                                      ),
                                    ),
                                  ...
                                  if (lunchMenu != null && lunchMenu.isNotEmpty)
                                    _LunchTimelineBlock(
                                      menu: lunchMenu,
                                      rowH: _rowH,
                                      sh: sh,
  ...
  List<Widget> _eventBlocks(
    ...
    for (final e in timed) {
      ...
      final draggable =
          idx >= 0 && !e.isTimetable && !e.sport && !e.academic && !e.birthday;
      blocks.add(
        _TimedEventBlock(
          event: e,
          index: idx,
  ...
  (String, int)? _findAnchorFor(
    ...
    for (final entry in events.entries) {
      for (var i = 0; i < entry.value.length; i++) {
        ...
        final fallback =
            virtual.id == null && e.t == virtual.t && e.tm == virtual.tm;
  ...
  void _shiftEventTime(EventItem e, int idx, int deltaMinutes, double height) {
    ...
    ref.read(eventsProvider.notifier).updateEvent(widget.dateKey, idx, updated);
  }

  // 종료 시각만 변경(리사이즈). 15분 스냅, 최소 15분 길이.
  void _resizeEvent(EventItem e, int idx, int deltaMinutes, double height) {
    if (idx < 0 || deltaMinutes == 0) return;
    final parts = e.tm!.split(':');
    final h = int.tryParse(parts[0]) ?? 0;
    final m = parts.length > 1 ? (int.tryParse(parts[1]) ?? 0) : 0;
  ...
  bool _isInCurrentWeek(DateTime date) {
    ...
    return du.isSameDay(currentMonday, targetMonday);
  }
}

class _LunchTimelineBlock extends StatelessWidget {
  final String menu;
  final double rowH;
  final SurlapColors sh;

  const _LunchTimelineBlock({
    required this.menu,
    required this.rowH,
    required this.sh,
  });
  ...
  @override
  Widget build(BuildContext context) {
    const color = Color(0xFFF2A93B);
    final displayMenu = getDisplaySubjectName(
      menu,
      lunch: true,
    ).replaceAll('\n', ' · ');
    return Positioned(
      ...
      height: rowH - 4,
      child: Semantics(
        label: '${tr('점심시간')}, $displayMenu',
        child: Container(
          padding: const EdgeInsets.symmetric(horizontal: Gap.sm),
          decoration: BoxDecoration(
            color: color.withValues(alpha: sh.dark ? 0.28 : 0.18),
            borderRadius: BorderRadius.circular(8),
          ...
          child: LayoutBuilder(
            builder: (context, constraints) {
              ...
              return Row(
                children: [
                  const Icon(Icons.restaurant_rounded, size: 17, color: color),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(Radii.card),
        border: Border.all(
          color: sh.accent.withValues(alpha: 0.22),
      ...
      child: Column(
        ...
        children: [
          ...
          ...items.map((e) {
            ...
            final icon = e.birthday ? Icons.cake_rounded : Icons.school_rounded;
            return Padding(
...
class _TimedEventBlockState extends State<_TimedEventBlock> {
  ...
  Widget build(BuildContext context) {
    ...
    final liveHeight = (widget.height + _resizeDy).clamp(20.0, double.infinity);
