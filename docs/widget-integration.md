...
- `ios/SurlapWidget/SurlapTheme.swift` — 위젯 디자인 토큰
- `ios/SurlapWidget/PeriodBar.swift` — 교시 세그먼트 바
- `ios/SurlapWidget/NowNextCard.swift` — 지금/다음 카드 모듈
- `ios/SurlapWidget/SurlapLiveActivity.swift` — Live Activity + Dynamic Island

### 1.2 Xcode 단계
1. Xcode에서 `ios/Runner.xcworkspace` 열기.
2. 좌측 트리에서 `SurlapWidget` 폴더 우클릭 → "Add Files to Runner..." → 위 4개 파일 선택.
   타깃은 **SurlapWidget** 만 체크(Runner 제외).
3. Signing & Capabilities → SurlapWidget 타깃 → Capabilities + 버튼 →
   **App Groups** + **Push Notifications**(Live Activity 원격 갱신용) 추가.
4. `Info.plist`에 `NSSupportsLiveActivities = YES` 추가(Runner 측, 메인 앱).
5. WidgetBundle 에 `SurlapLiveActivity()` 추가 — 기존 `SurlapWidget.swift` 의
   `SurlapWidgetBundle` 안에 한 줄.
   ```swift
   @main
   ...
   ```
6. App Group 식별자 통일 — 현재 `group.com.kev208dev.Surlap`. 새 브랜드에 맞게
   `group.com.kev208dev.Surlap` 등으로 바꾸려면:
   - SurlapWidget.entitlements + Runner.entitlements 둘 다 갱신
...
### iOS Widget 타깃
1. `assets/fonts/Pretendard-Bold.otf` 등을 `ios/SurlapWidget/Fonts/` 로 복사.
2. Xcode → SurlapWidget 타깃 → "Build Phases" → "Copy Bundle Resources" 에 추가.
